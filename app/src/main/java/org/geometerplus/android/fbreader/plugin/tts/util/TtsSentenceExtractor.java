package org.geometerplus.android.fbreader.plugin.tts.util;
...
public class TtsSentenceExtractor {
    ...
    public static SentenceIndex[] build(List<String> wl, List<Integer> il, TextToSpeech currentTTS, boolean wordsOnly) {
        Locale loc = null;
        if (currentTTS != null) loc = currentTTS.getLanguage();
        if (loc == null)
            loc = Locale.getDefault();
        int breakSentences = 200; // other engines have troubles over 5000 characters...
        try {
